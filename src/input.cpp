...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  bool detail::try_add_input_delta(std::int16_t lhs, std::int16_t rhs, std::int16_t &result) {
  ...
  batch_result_e batch(PNV_REL_MOUSE_MOVE_PACKET dest, PNV_REL_MOUSE_MOVE_PACKET src) {
    ...
    if (!detail::try_add_input_delta(util::endian::big(dest->deltaX), util::endian::big(src->deltaX), deltaX)) {
      return batch_result_e::terminate_batch;
    }
    if (!detail::try_add_input_delta(util::endian::big(dest->deltaY), util::endian::big(src->deltaY), deltaY)) {
      return batch_result_e::terminate_batch;
    }
  ...
  batch_result_e batch(PNV_SCROLL_PACKET dest, PNV_SCROLL_PACKET src) {
    std::int16_t scrollAmt;

    // Batching is safe as long as the result doesn't overflow a 16-bit integer
    if (!detail::try_add_input_delta(util::endian::big(dest->scrollAmt1), util::endian::big(src->scrollAmt1), scrollAmt)) {
      return batch_result_e::terminate_batch;
    }
  ...
  batch_result_e batch(PSS_HSCROLL_PACKET dest, PSS_HSCROLL_PACKET src) {
    ...
    // Batching is safe as long as the result doesn't overflow a 16-bit integer
    if (!detail::try_add_input_delta(util::endian::big(dest->scrollAmount), util::endian::big(src->scrollAmount), scrollAmt)) {
      return batch_result_e::terminate_batch;
    }
